...
function _validateWebsiteConfigXml(jsonResult, xml, log, cb) {
    ...
    if (resultConfig.RedirectAllRequestsTo && (resultConfig.IndexDocument ||
    resultConfig.ErrorDocument || resultConfig.RedirectAllRequestsTo)) {
        errMsg = 'RedirectAllRequestsTo cannot be provided in conjunction ' +
        ...
        for (let i = 0; i < parent[0].RoutingRule.length; i++) {
            ...
            if (Array.isArray(rule.Condition) && rule.Condition.length === 1) {
                ...
                if (rule.Condition[0].HttpErrorCodeReturnedEquals) {
                    ...
                    if (isNaN(code) || !(code > 399 && code < 600)) {

...
function _getPartKey(uploadId, paddedPartNumber) {
    return `${uploadId}${constants.splitter}${paddedPartNumber}`;
}
...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    const partKey = _getPartKey(uploadId, paddedPartNumber);
    const mpuOverviewKey = _getOverviewKey(request.objectKey, uploadId);
    ...
    return async.waterfall([
        ...
        next => metadata.getObjectMD(mpuBucketName, mpuOverviewKey, log,
            (err, res) => {
                if (err && (err.NoSuchBucket || err.NoSuchKey)) {
        ...
        next => metadata.getObjectMD(mpuBucketName, partKey, log,
            (err, res) => {
                ...
                // If there is a pre-existing part, get the part size for
                // Utapi metrics.
                if (res) {

...
export default class BucketInfo {
    ...
    * @param {object} websiteConfiguration - website configuration
    * @param {string} websiteConfiguration.indexDocument -
    * object key to serve as index for bucket website
    * @param {string} websiteConfiguration.errorDocument -
    * object key to serve as custom error document for 4xx errors
    * @param {object} websiteConfiguration.redirectAllRequestsTo -
    * object to specify where to redirect all requests to bucket website to
    * @param {string} websiteConfiguration.redirectAllRequestsTo.hostName -
    * hostname to use in redirect request
    * @param {string} websiteConfiguration.redirectAllRequestsTo.protocol -
    * (optional) protocol to use for redirect request ('http' or 'https')
    * @param {object[]} websiteConfiguration.routingRules -
    * collection of rules to redirect certain requests to bucket website
    * @param {object} websiteConfiguration.routingRules[].redirect -
    * describe how to redirect requests to bucket website for a given rule
    * @param {string} websiteConfiguration.routingRules[].redirect.hostName -
    * hostname to use in redirect request
    * @param {string} websiteConfiguration.routingRules[]
    .redirect.httpRedirectCode - hostname to use in redirect request
    * @param {string} websiteConfiguration.routingRules[].redirect.protocol -
    protocol to use for redirect request ('http' or 'https')
    * @param {string} websiteConfiguration.routingRules[]
    .redirect.replaceKeyWith - specific object key to use in redirect request
    * @param {string} websiteConfiguration.routingRules[].redirect
    .replaceKeyPrefixWith - object key prefix to use in redirect request
    * @param {object} websiteConfiguration.routingRules[].condition -
    * describe conditions for redirecting requests to bucket website for a rule
    * @param {string} websiteConfiguration.routingRules[].condition
    .httpErrorCodeReturnedEquals - http err code under which to apply redirect
    * @param {string} websiteConfiguration.routingRules[].condition
    .keyPrefixEquals - object key name prefix when the redirect is applied
    */
    constructor(name, owner, ownerDisplayName, creationDate,
        ...
        if (websiteConfiguration) {
            assert.strictEqual(typeof websiteConfiguration, 'object');
    ...
    */
    serialize() {
        const bucketInfos = {
            ...
            websiteConfiguration: this._websiteConfiguration,
        };
    ...
    static deSerialize(stringBucket) {
        ...
        return new BucketInfo(obj.name, obj.owner, obj.ownerDisplayName,
            ...
            obj.versioningConfiguration, obj.websiteConfiguration);
    }

...
export function clientCheck(statsClient, log, cb) {
    const clients = [
        data,
        metadata,
        vault,
    ...
    clientTasks.push(
         (done) => {
            statsClient.getStats(log, (err, res) => {
                if (err) {
                    return done(err);
                }
                return done(null, { s3: res });
            });
        }
    );
...
export function healthcheckHandler(clientIP, deep, req, res, log,
    ...
    console.log('statsClient!!!', statsClient)
